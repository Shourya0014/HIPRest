...
from basicsr.models.model_utils import ExponentialMovingAverage
from tqdm import tqdm

from basicsr.models.archs import define_network
#updated code below
sys.path.append(r"D:\intel\Restormer\basicsr\models\archs")
...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            #//edited code
            self.teacher= Restormer()
            teacher_ckpt=torch.load('D:\intel\Restormer\Motion_Deblurring\pretrained_models\motion_deblurring.pth')
            self.teacher.load_state_dict(teacher_ckpt['params'],strict=True)
            self.teacher.eval()
            self.teacher.to(self.device)
            #till here above
            self.init_training_settings()
        if self.is_train and self.opt['train'].get('ema_decay', 0) > 0:
            self.net_g_ema = deepcopy(self.net_g)
            self.ema_decay = self.opt['train']['ema_decay']
            self.ema = ExponentialMovingAverage(self.net_g_ema.parameters(), decay=self.ema_decay)
            self.net_g_ema.to(self.device)
            #till here
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('distill_opt'):
            distill_type = train_opt['distill_opt'].pop('type')
            distill_cls = getattr(loss_module, distill_type)
            self.cri_distill = distill_cls(**train_opt['distill_opt']).to(self.device)
        else:
    ...
    #     self.log_dict = self.reduce_loss_dict(loss_dict)
        self.optimizer_g.zero_grad()
        with torch.no_grad():
            teacher_pred = self.teacher(self.lq)
            if isinstance(teacher_pred, list):
                teacher_pred = teacher_pred[-1]

        student_preds = self.net_g(self.lq)
        if not isinstance(student_preds, list):
            student_preds = [student_preds]

        self.output = student_preds[-1]

        # Total loss = distillation + optional GT loss
        l_total = 0
        loss_dict = OrderedDict()
        ...
        if self.cri_pix:
            ...
            for pred in student_preds:
                l_pix += self.cri_pix(pred,teacher_pred, self.gt)
            l_total += l_pix
            loss_dict['l_pix'] = l_pix

        if self.cri_distill:
            l_distill = self.cri_distill(self.output, teacher_pred)
            l_total += l_distill
        ...
        #addded code part
        if hasattr(self, 'ema'):
            self.ema.update(self.net_g.parameters())
